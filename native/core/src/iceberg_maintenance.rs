    ...
    match mode {
        ...
        PrefixMismatchMode::Error => {
            for candidate in valid {
                if !component_matches(candidate.scheme.as_deref(), actual_ident.scheme.as_deref()) {
...
async fn delete_paths_with<F, Fut>(paths: Vec<Path>, max_concurrent_deletes: usize, delete: F)
where
    F: Fn(Path) -> Fut,
...
fn execute_remove_orphans(args: RemoveOrphansArgs) -> Result<Vec<String>, ExecutionError> {
    ...
    let (mut orphan_candidates, conflicts) = get_runtime().block_on(async {
        ...
        Ok::<(Vec<(String, Path)>, HashSet<String>), ExecutionError>((orphan_candidates, conflicts))
    })?;
