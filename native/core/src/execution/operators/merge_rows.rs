...
pub struct MergeRowsStream {
    ...
    /// Emitted-batch count. `output_rows / output_batches` is this operator's average output
    /// batch size -- a fragmented merge output slows the downstream writer even when the writer
    /// itself is fast, so this is the number to check first when a MERGE's write phase is slow.
...
mod tests {
    ...
    fn keep_matched_discard_rest() {
        ...
        let config = test_config(vec![keep_all()], vec![keep_all()], vec![discard_all()], None);
