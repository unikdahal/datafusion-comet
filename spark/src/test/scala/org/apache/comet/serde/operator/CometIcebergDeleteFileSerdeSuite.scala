...
/**
 * Locks in fail-loud behavior for required delete-file metadata. On supported Iceberg versions,
 * required accessors must resolve and invoke successfully once native execution has been selected.
 * A null equality-field-id list is still valid for position deletes.
 */
class CometIcebergDeleteFileSerdeSuite extends AnyFunSuite {
  ...
  test("Iceberg delete content names match native serde literals") {
    assert(
      FileContent.POSITION_DELETES.toString == IcebergReflection.ContentTypes.POSITION_DELETES)
    assert(
      FileContent.EQUALITY_DELETES.toString == IcebergReflection.ContentTypes.EQUALITY_DELETES)
  }
  ...
  class PositionDeleteFile extends BaseDeleteFile {
  ...
  class EqualityDeleteFile extends BaseDeleteFile {
  ...
  class EqualityDeleteFileWithNullIds extends BaseDeleteFile {
  ...
  class EqualityDeleteFileWithEmptyIds extends BaseDeleteFile {
  ...
  class ThrowingContentDeleteFile extends BaseDeleteFile {
  ...
  class ThrowingSpecIdDeleteFile extends BaseDeleteFile {
    override def location(): String = "s3://bucket/d.parquet"
    def content(): String = "POSITION_DELETES"
  ...
  class ThrowingEqualityIdsDeleteFile extends BaseDeleteFile {
    override def location(): String = "s3://bucket/d.parquet"
    def content(): String = "EQUALITY_DELETES"
    def specId(): Int = 0
  ...
  class NoContentAccessorDeleteFile extends BaseDeleteFile {
    override def location(): String = "s3://bucket/d.parquet"
    def specId(): Int = 0
    def equalityFieldIds(): java.util.List[Integer] = null
  }

  class NoEqualityIdsAccessorDeleteFile extends BaseDeleteFile {
    override def location(): String = "s3://bucket/d.parquet"
    def content(): String = "EQUALITY_DELETES"
    def specId(): Int = 0
  }
