 ...
 * `nullable` / `containsNull` / `valueContainsNull`). DataFusion's `greatest`/`least` coerce every
 * argument to one common type; Comet's planner then inserts a `CastExpr` for any argument whose
 * Arrow type differs from that common type -- differing nested nullability, but also a struct/list
 * field *name* that Spark's `DataType` cannot express (e.g. `greatest(array_repeat(n, 1).e,
 * array_repeat(n.e, 1))`, whose elements are `list<e: struct<>>` vs `list<item: struct<>>`).
 * Casting a zero-field struct errors, so decline any multi-argument call whose argument types
 * contain an empty struct; a single argument is never coerced against anything. See
 * `SupportLevel.containsEmptyStruct` and https://github.com/apache/datafusion-comet/pull/5414.
 */
