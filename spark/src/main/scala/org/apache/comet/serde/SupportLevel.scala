...
object SupportLevel {
   ...
   * `CastExpr` for any Arrow-level difference -- including a struct/list field *name* that Spark's
   * `DataType` cannot express. Casting a zero-field struct fails with "no field name overlap", so
   * such lookups stay on Spark. See https://github.com/apache/datafusion-comet/pull/5414.
   */
