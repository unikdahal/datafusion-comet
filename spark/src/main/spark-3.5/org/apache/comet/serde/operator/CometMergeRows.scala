...
object CometMergeRows extends CometOperatorSerde[MergeRowsExec] {
  ...
  override def convert(
    ...
    def convertInstruction(instr: MergeRows.Instruction): Option[MergeInstruction] = {
      if (!instructionShapeSatisfied(instr, op.output.size)) {
        return None
      }
  ...
  private val instructionShapeFallbackReason: String =
    "MERGE instruction must be Discard, Keep, or Split with output rows matching the plan width"
  ...
  private def instructionShapesSatisfied(op: MergeRowsExec): Boolean = {
    val outputWidth = op.output.size
    op.matchedInstructions.forall(instructionShapeSatisfied(_, outputWidth)) &&
    op.notMatchedInstructions.forall(instructionShapeSatisfied(_, outputWidth)) &&
    op.notMatchedBySourceInstructions.forall(instructionShapeSatisfied(_, outputWidth))
  }

  private def instructionShapeSatisfied(
      instruction: MergeRows.Instruction,
      outputWidth: Int): Boolean = {
    instruction.outputs.size <= 2 && instruction.outputs.forall(_.size == outputWidth)
  }
