...
object CometMergeRows extends CometOperatorSerde[MergeRowsExec] {
  ...
  override def convert(
    ...
    def convertInstruction(instr: MergeRows.Instruction): Option[MergeInstruction] = {
      ...
      val actionContext = mergeActionContext(instr)
      ...
      val contextMatchesShape = (instr.outputs.size, actionContext.isDefined) match {
        case (1, true) => true
        case (0 | 2, false) => true
        case _ => false
      }
      if (condition.isDefined && outputs.forall(_.isDefined) && contextMatchesShape) {
        val instructionBuilder = MergeInstruction
          .newBuilder()
          .setCondition(condition.get)
          .addAllOutputs(outputs.map(_.get).asJava)
        actionContext.foreach(instructionBuilder.setContext)
        Some(instructionBuilder.build())
      } else {
        None
      }
    }
  ...
  private def instructionShapeSatisfied(
    ...
    val contextMatchesShape =
      (instruction.outputs.size, mergeActionContext(instruction).isDefined) match {
        case (1, true) => true
        case (0 | 2, false) => true
        case _ => false
      }
    contextMatchesShape && instruction.outputs.size <= 2 && instruction.outputs.forall { row =>
